          ...
          {!hasDemo && (
            <div className="mt-11">
              <button
                ...
                onClick={tryDemo}
                disabled={demoBusy}
                className="rounded-full border border-[#b9915c]/45 bg-[#fff0cf] px-7 py-3 text-base font-medium text-[#4b3828] shadow-[0_0_30px_rgba(255,218,162,0.72),inset_0_2px_2px_rgba(255,255,255,0.8)] transition duration-400 hover:scale-[1.03] hover:bg-[#fff6e0] disabled:cursor-not-allowed disabled:opacity-50 sm:px-8"
              >
                {demoBusy ? 'Loading demo...' : 'Try the demo: Leo, age 4'}
              </button>
              {demoError && <p className="mt-4 text-sm font-medium text-[#ffb08f]">{demoError}</p>}
            </div>
          )}
        </section>
      </main>
